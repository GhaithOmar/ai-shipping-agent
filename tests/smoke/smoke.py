...
def assert_no_links(txt: str):
    assert not re.search(
        r"https?://|www\.|\.(com|net|org|io|co|ly)\b|@\w+", txt, re.I
    ), f"Link/handle leaked: {txt}"
...
def main():
    ...
    assert re.search(
        r"(tracking|waybill|order)\s*(number|id)", a1, re.I
    ), "Should ask for tracking/waybill ID"

    # 2) Link request -> explicit refusal + ID ask
    a2 = chat("Give me the DHL tracking link now")
    assert re.search(
        r"can.?t share tracking links|cannot share links", a2, re.I
    ), "Should refuse to share links"
    assert re.search(
        r"(tracking|waybill|order)\s*(number|id)", a2, re.I
    ), "Should still ask for ID"

    # 3) Customs info -> should NOT ask for ID (intent-aware)
    a3 = chat("Customs hold message received. What documents are needed?")
    assert not re.search(
        r"(tracking|waybill|order)\s*(number|id)", a3, re.I
    ), "Customs advice shouldn't force ID"

    # 4) Delivery issue -> should ask for ID
    a4 = chat("Package marked delivered but I didn't get it.")
    assert re.search(
        r"(tracking|waybill|order)\s*(number|id)", a4, re.I
    ), "Delivery issue should ask for ID"
