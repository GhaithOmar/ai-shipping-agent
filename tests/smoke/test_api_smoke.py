...
def offline_generate(
    user_msg: str, top_k_context: List[str], provided_tracking: Optional[str] = None
) -> str:
...
def test_chat_ok():
    r = client.post(
        "/chat", json={"message": "Track order 12345678 with Shipping_A", "top_k": 2}
    )
...
def test_chat_stream_minimal():
    r = client.post(
        "/chat/stream", json={"message": "Return policy for fragile items", "top_k": 2}
    )
    assert r.status_code == 200
    js = r.json()
