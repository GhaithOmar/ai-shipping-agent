...
def test_chat_agent_branch_monkeypatched(client, monkeypatch):
    ...
    def offline_guarded(
        user_msg, top_k_context, tok=None, model=None, provided_tracking=None
    ):
        parts = []
    ...
    def gen_fn(msg, ctx, tid=None):  # will call patched appmod.infer_guarded
        return appmod.infer_guarded(
            msg, ctx, tok=None, model=None, provided_tracking=tid
        )

    appmod.agent_app = build_graph(gen_fn)

    payload = {
        "message": "Track order 12345 with Shipping_A and give last two scan events."
    }
