...
def detwitterize_agent(text: str, prepend_steps: bool) -> str:
    ...
    if (
        prepend_steps
        and "Here's what I'll do:" not in t
        and "Here’s what I’ll do:" not in t
    ):
        t = (
...
def load_kaggle(path: str, mapping: dict) -> pd.DataFrame:
    df = pd.read_csv(
        path,
        dtype={
            mapping["tweet_id_col"]: "string",
            mapping["response_tweet_id_col"]: "string",
        },
    ...
    user_col, inbound_col, tweet_id_col, resp_id_col = (
        mapping["user_col"],
        mapping["inbound_col"],
        mapping["tweet_id_col"],
        mapping["response_tweet_id_col"],
    )
    ...
    # customers: inbound & has a non-empty reply id
    customers = df.loc[
        df[inbound_col] & df[resp_id_col].ne(""), [tweet_id_col, resp_id_col, user_col]
    ].rename(columns={user_col: "input_raw"})

    # companies: NOT inbound
    companies = df.loc[~df[inbound_col], [tweet_id_col, user_col]].rename(
        columns={tweet_id_col: "reply_tweet_id", user_col: "assistant_raw"}
    )
    ...
    ).dropna(subset=["assistant_raw"])
    return pairs[["input_raw", "assistant_raw"]]


def load_hf(repo_id: str, split: str, mapping: dict) -> pd.DataFrame:
    ds = load_dataset(repo_id, split=split)
    return ds.to_pandas()[[mapping["user_col"], mapping["agent_col"]]].rename(
        columns={
            mapping["user_col"]: "input_raw",
            mapping["agent_col"]: "assistant_raw",
        }
    )
...
def main():
    ...
    mini_size = int(cfg["output"]["mini_size"])
    val_ratio = float(cfg["output"]["val_ratio"])
    ...
    for ds_cfg in cfg["datasets"]:
        if ds_cfg["kind"].startswith("kaggle"):
            ...
            continue
        # filter
        mask = df["input_raw"].astype(str).apply(
            lambda s: passes_shipping_filter(s, keywords)
        ) | df["assistant_raw"].astype(str).apply(
            lambda s: passes_shipping_filter(s, keywords)
        )
        df = df[mask]
        for _, row in df.iterrows():
            all_examples.append(
                map_pair(str(row["input_raw"]), str(row["assistant_raw"]), cfg)
            )
    ...
    # --- Rebalancing ---
    ratios = cfg.get(
        "ratios", {"bitext_hf": 0.7, "kaggle_twitter": 0.2, "synthetic": 0.1}
    )
    buckets = {k: [] for k in ratios}
    for ex in all_examples:
        src = (
            "bitext_hf"
            if "bitext" in ex["assistant_response"].lower()
            else "kaggle_twitter"
        )
