...
def _billable_weight(
    kg: float, dims_cm: Optional[Tuple[float, float, float]] = None
) -> float:
...
def rate_quote(
    ...
    mult = {"express": 1.6, "standard": 1.0, "economy": 0.8}.get(
        (service_level or "standard").lower(), 1.0
    )
